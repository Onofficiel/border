...
fill: var(--border-secondary);
			...
			if (!isConfigFileExisting) {
				...
				this.#Files.style = await w96.FS.readstr(
					`c:/user/appdata/Border/themes/${this.#Files.config.theme.currentTheme
					}`
				);
	...
	handleURI(url) {
		if (
			...
			!RegExp(`^(?:${Array.from(this.protocols.keys()).join("|")}|data):/*`).test(url)
		)
			return [
				"data:text/html," +
...
`;
		...
		let page = [
            ...
            <b>Primary : </b><input class="border-setup-primary-input" type="color" value="${this.#Files.config.theme.primary
					}"></input><br>
            <b>Secondary : </b><input class="border-setup-secondary-input" type="color" value="${this.#Files.config.theme.secondary
					}"></input>
            `,
				save: [
					{
            ...
            <b>Height : </b><input class="border-setup-height-input" type="number" value="${this.#Files.config.window.height
					}"></input>px<br>
            <b>Width : </b><input class="border-setup-width-input" type="number" value="${this.#Files.config.window.width
					}"></input>px<br>
            <b>Use the custom titlebar : </b><input class="border-setup-titlebar-input" type="checkbox" checked="${this.#Files.config.window.customTitlebar
					}"></input>
            `,
				save: [
					{
			...
			},
			{
				content: `
            <h1>Others</h1>
            <b>Default page : </b><input class="border-setup-default-input" type="url" value="${this.#Files.config.browser.defaultPage
					}"></input><br>
            <b>Enable shortcuts ? </b><input class="border-setup-shortcuts-input" type="checkbox" checked="${this.#Files.config.browser.enableShortcuts
					}"></input><br>
	...
	async #main() {
		...
		await blugins.forEach(async (path) => {
		...
		h[2].addEventListener(
			"click",
			() => (this.Tab.current.webview.src = this.Tab.current.webview.src)
		);
